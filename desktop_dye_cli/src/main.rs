...
async fn main() -> Result<()> {
    ...
    loop {
        let res = capture_and_submit(&api, &config, target_screen).await;
        if let Err(e) = res {
            failures += 1;
            ...
            failures = 0;
            println!(
                "Waiting {} second(s) for next capture...",
                config.capture_interval
            );
            tokio::time::sleep(std::time::Duration::from_secs(config.capture_interval)).await;
        }
    }
}
